...
export function UpcomingPage() {
  ...
  const fetchQuests = async () => {
    try {
      ...
      // Filter for future quests (dueDate > today)
      const futureQuests = data
        .filter((q) => {
      ...
      setQuests(futureQuests);
    } catch (err) {
      console.error('Failed to load upcoming quests', err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const groupedQuests = quests.reduce(
    ...
    {} as Record<string, QuestResponse[]>
  );
