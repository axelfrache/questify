...
export function LoginPage() {
  ...
  const handleOidcLogin = async () => {
    ...
    try {
      ...
      setError(t('login.redirect_failed'));
    }
  };
  ...
  return (
    <div className="flex min-h-svh items-center justify-center bg-background p-4 animate-in fade-in duration-500 relative">
      ...
      <div className="w-full max-w-sm space-y-6">
        ...
        {oidcEnabled ? (
          <Button className="w-full" onClick={handleOidcLogin} disabled={isRedirecting}>
            {isRedirecting ? (
              <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" /> {t('login.redirecting')}
              </>
            ) : (
              t('login.submit')
            )}
