...
public class AuthService {
  ...
  public AuthResponse register(RegisterRequest request) {
    Span.current().setAttribute("questify.auth.flow", "register");
    if (userRepository.existsByEmail(request.email())
        || userRepository.existsByUsername(request.username())) {
      Span.current().setAttribute("questify.auth.result", "duplicate");
  ...
  public AuthResponse login(LoginRequest request) {
    Span.current().setAttribute("questify.auth.flow", "login");
    try {
  ...
  public AuthResponse refresh(RefreshTokenRequest request) {
    Span.current().setAttribute("questify.auth.flow", "refresh");
    var refreshToken =
  ...
  public void logout(RefreshTokenRequest request) {
    Span.current().setAttribute("questify.auth.flow", "logout");
  ...
  private static void setUserAttributes(User user) {
    ...
    Span.current().setAttribute("questify.user.enabled", user.isEnabled());
  }
}
